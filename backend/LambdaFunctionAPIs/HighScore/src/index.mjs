...
const getScoreAndRank = async (client, userId, gameId) => {
    try {
        let scoreQuery, rankQuery, queryParams;
        
        if (gameId === 7 || gameId === 5) {
            // For game_id = 7, get the lowest score and rank based on ascending order
            scoreQuery = 'SELECT MIN(score) AS target_score FROM game_scores WHERE user_id = $1 AND game_id = $2';
            rankQuery = 'SELECT COUNT(*) + 1 AS rank FROM game_scores WHERE game_id = $1 AND score < $2';
        } else {
            // For other games, get the highest score and rank based on descending order
            scoreQuery = 'SELECT MAX(score) AS target_score FROM game_scores WHERE user_id = $1 AND game_id = $2';
            rankQuery = 'SELECT COUNT(*) + 1 AS rank FROM game_scores WHERE game_id = $1 AND score > $2';
        }
...
export const handler = async (event) => {
    ...
    try {
        ...
        const { targetScore, rank } = await getScoreAndRank(client, user_id, game_id);
